...
#### 3.4.1 Publish Workflow — PR-based Squash Merge
...
- [ ] **Backend: PR-based publish** — Replace `publishBranch()` in `services/sources/github.ts`:
  ...
  - On successful merge + keep branch: reset working branch ref to base HEAD via Git Refs API
...
- [x] **Production OAuth apps** — create/update OAuth client registrations with production redirect URIs (`https://api.notebookmd.io/auth/oauth/*/callback`):
  - [x] Microsoft (Entra ID): new app registration, multi-tenant (personal + work), redirect URI configured
  - [x] Google Cloud: new OAuth client, authorized redirect URI + JS origin configured
  - [x] GitHub: new production OAuth app, callback URL configured
  - [x] GitHub App: existing `notebook-md` app with production callback + webhook URLs
...
- [x] Bundle analysis: install rollup-plugin-visualizer, generate treemap of 1.6MB bundle
- [x] Code splitting: route-level lazy loading (React.lazy + Suspense) for app vs marketing vs admin
- [x] Vendor chunk splitting: configure Vite manualChunks for react, tiptap/prosemirror, katex, lowlight
- [x] KaTeX font optimization: assessed — already lazy-loaded by browser via `@font-face`, no user-facing impact
- [x] Image optimization: compress og-image.png (313KB → 107KB)
- [x] Web Vitals + PostHog: send LCP/INP/CLS/FCP/TTFB as PostHog events
...
- [x] Static asset cache headers: verified production sets Cache-Control immutable for hashed assets
