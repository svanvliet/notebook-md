...
- [x] Set up Vitest + React Testing Library for apps/adminAdd vitest, @testing-library/react, @testing-library/jest-dom, @testing-library/user-event, jsdom to dev deps
...
- [x] Migration: Add last_active_at column to users table (updated on session creation/refresh)ALTER TABLE users ADD COLUMN last_active_at TIMESTAMPTZ;
...
- [x] GET /admin/users — Add query params:sort (name, email, created_at, last_active_at) + order (asc, desc)
...
- [x] UserPicker (apps/admin/src/components/ui/UserPicker.tsx)Headless UI Combobox with async search
...
- [x] SlidePanel with tabbed content (wide variant, ~50% viewport):Overview tab — Email, display name, joined date, last active, 2FA status, admin badge, identity providers list, quick actions (Suspend/Unsuspend, Force Logout, Delete)

-   **Notebooks tab** — Table of user's notebooks (name, source type, file count, created date)
    
-   **Flags & Flights tab** — Resolved flag table (flag key, value, source: override/flight/kill-switch/not\_delivered), groups list, flights list, per-user overrides with edit/delete
...
- [x] admin.test.ts — Add tests:User search endpoint (/admin/users/search?q=) — returns matches, limits to 10, empty query

-   User list sorting (sort by name asc/desc, by created\_at)
...
- [x] Migration: Add archived column to feature_flags tableALTER TABLE feature_flags ADD COLUMN archived BOOLEAN DEFAULT false;
...
- [x] GET /admin/feature-flags — Add to response:flights[] — array of {id, name} for flights containing this flag
...
- [x] Add "Feature Management" section to sidebar with collapsible sub-nav:🚩 Feature Flags (/features/flags)
...
- [x] Flight detail page (/features/flights/:id) — Full page layout:Header: Name, status badge, enable/disable toggle
...
- [x] Pagination (DataTable, 20 per page)
- [x] Markdown preview — Split view: edit (textarea) on left, rendered preview on rightUse a lightweight markdown renderer (reuse marked from web app or add dependency)
...
| --- | --- | --- |
| `apps/api/src/tests/admin.test.ts` | 2 | User search, sorting, status filter, force-logout, last\_active\_at |
