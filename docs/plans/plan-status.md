...
-   **Open Graph tags:** og:title, og:description, og:image (with dimensions and alt), og:url, og:type, og:site\_name, og:locale
    
-   **Twitter Card tags:** summary\_large\_image with title, description, image, and alt
...
| --- | --- | --- | --- |
| node\_modules | `node-modules-${{ hashFiles('package-lock.json') }}` | Reuses install job's cached `node_modules` | ~43s (skip `npm ci`) |
...
-   `8f044be` — perf: cache node\_modules and Playwright browsers in E2E job
