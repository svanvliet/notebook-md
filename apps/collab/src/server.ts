...
const server = Server.configure({
  ...
  async onAuthenticate({ token, documentName }) {
    ...
    // Validate session via database (same as API auth middleware)
    ...
    if (notebookResult.rows[0].user_id === user.user_id) {
      permission = 'owner';
    } else {
    ...
    return {
      user: {
        ...
        color: assignColor(user.user_id),
        permission,
      },
    };
  },
